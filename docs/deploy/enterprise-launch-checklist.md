...
- [ ] Paddle Checkout **Default payment link** is the marketing apex `/billing/paddle-return` (Paddle stores HTTPS always; sandbox overlay UAT can leave `https://localhost:8088/billing/paddle-return`; hosted local UAT uses the ngrok HTTPS apex; production: `https://cohestra.app/billing/paddle-return`) — never a tenant slug
