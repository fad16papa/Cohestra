/** Desktop grid: checkbox · contact · status · last reg · last outreach · actions (3 icons) */
export const clientsTableGridClassName =
  "grid grid-cols-1 gap-3 px-4 py-3 sm:grid-cols-[2rem_minmax(13rem,1.5fr)_6.75rem_minmax(0,1.15fr)_minmax(0,0.85fr)_7.75rem] sm:items-center sm:gap-3 sm:px-4 sm:py-3";

export const clientsTableCheckboxColumnClassName =
  "hidden sm:flex sm:items-center sm:justify-center";

export const clientsTableStatusColumnClassName =
  "flex min-w-0 items-center justify-start overflow-hidden";

export const clientsTableTextColumnClassName =
  "min-w-0 overflow-hidden";

export const clientsTableActionsColumnClassName =
  "flex min-w-0 items-center gap-1 sm:justify-end";
