...
public sealed class RedisAuthOtpVerifyRateLimiter(
    IConnectionMultiplexer redis,
    IOptions<AuthOtpVerifyRateLimitOptions> options) : IAuthOtpVerifyRateLimiter
{
    private const string LimiterName = "AuthOtpVerify";
    ...
    public async Task<bool> AllowVerifyAsync(
        string email,
        string? clientIdentifier,
        CancellationToken cancellationToken = default)
    {
        var settings = options.Value;
        if (settings.MaxFailedAttemptsPerWindow <= 0)
        {
            return true;
        }
        ...
        if (!await EvaluateAllowAsync(
                ...
                now,
                window,
                settings.MaxFailedAttemptsPerWindow))
        {
            return false;
        }
    ...
    public async Task RecordFailedVerifyAsync(
        string email,
        string? clientIdentifier,
        CancellationToken cancellationToken = default)
    {
    ...
    {
        var db = redis.GetDatabase();
        var keys = new List<RedisKey> { BuildEmailKey(email) };
        if (!string.IsNullOrWhiteSpace(clientIdentifier))
        {
    ...
    private static RedisKey BuildEmailKey(string email)
    {
        ...
        return (RedisKey)$"auth:otp:verify:fail:email:{hash}";
    }
    ...
    private static RedisKey BuildIpKey(string clientIdentifier)
    {
        ...
        return (RedisKey)$"auth:otp:verify:fail:ip:{hash}";
    }
