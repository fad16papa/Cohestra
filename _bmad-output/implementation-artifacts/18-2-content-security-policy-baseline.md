---
epic: 18
story: 2
status: review
baseline_commit: 96cb862
---

# Story 18.2: Content-Security-Policy baseline

Status: review

## Story

As a **platform operator**,
I want **a documented, non-breaking CSP baseline for the web app**,
So that **XSS blast radius is reduced without breaking Next.js or the admin builder**.

## Context

Deferred from Story 17.4 CR decision — CSP can break Next.js inline scripts/styles. Ship report-only or permissive enforce mode first; document tighten path.

## Acceptance Criteria

1. **Given** production web responses  
   **When** CSP is enabled  
   **Then** `Content-Security-Policy` or `Content-Security-Policy-Report-Only` header is present

2. **Given** local Docker smoke  
   **When** login, dashboard, builder, public registration tested  
   **Then** no CSP violations block core flows

3. **Given** deploy docs  
   **When** operator reads CSP section  
   **Then** report-only → enforce upgrade path is documented

## Tasks / Subtasks

- [x] **Task 1 — Policy design**
  - [x] Audit Next.js requirements (inline scripts, `unsafe-inline` for styles if needed, font/img/connect-src)
  - [x] Choose report-only vs enforce for v1 (recommend report-only first ship)

- [x] **Task 2 — Implementation**
  - [x] Add CSP in header owner layer (coordinate with Story 18.3 — likely nginx for prod, Next for dev)
  - [x] `web/next.config.ts` and/or `deploy/nginx/app-ssl.conf.template`

- [x] **Task 3 — Verification**
  - [x] Manual smoke checklist in story or `docs/deploy/enterprise-launch-checklist.md` CSP row
  - [x] Optional: Playwright smoke if existing harness covers surfaces (skipped — no Playwright harness in repo)

## Dev Notes

- Next.js App Router often needs `'unsafe-inline'` for styles in dev; production may differ
- Do not duplicate CSP from both nginx and Next in prod (Story 18.3)
- API responses generally do not need CSP (JSON/HTML email separate)

## Dev Agent Record

### Completion Notes

- v1 ships **Content-Security-Policy-Report-Only** (non-blocking) with permissive directives for Next.js inline assets, Google Fonts, reCAPTCHA, Unsplash images, and Nominatim geocoding.
- Production: nginx `add_header` + `proxy_hide_header` (Story 18.3 pattern). Dev: Next.js via `content-security-policy.ts` dev variant (includes `ws:`/`wss:` for HMR).
- `npm run build` passes.

## File List

- `web/content-security-policy.ts`
- `web/next.config.ts`
- `deploy/nginx/app.conf`
- `deploy/nginx/app-ssl.conf.template`
- `docs/deploy/enterprise-launch-checklist.md`

## Change Log

- 2026-08-01: Story 18.2 — CSP report-only baseline; nginx prod, Next dev-only.
